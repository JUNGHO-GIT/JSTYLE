...
var dynamicStyle = function () {
  ...
  function checkValidClass(className) {
    ...
    return (
      ...
      !isNaN(value) &&
      parts.length === 2 &&
      Object.hasOwnProperty.call(stylesNumber, prefix)
    );
  }
  ...
  var numberSelector = Object.keys(stylesNumber)
    .map(function (prefix) {
      return '[class*="' + prefix + '-"][class*="' + prefix + '-\\d"]';
    })
    .join(", ");

  var stringSelector = Object.keys(stylesString)
    .map(function (prefix) {
      return "." + prefix;
    })
    .join(", ");
